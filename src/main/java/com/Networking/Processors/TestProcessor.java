...
public class TestProcessor implements Processable {
    ...
    public TestProcessor(ChangeableModel model, ClientUser user, Logic logic) {
        this.model = model;
        this.user = user;
        this.logic = logic;
    }
    ...
    public void close() {
        user.drop();
        logic.notify(ACTIONS.DISCONNECTED, null);
    }
    ...
    protected boolean onIncomingMessage(AbstractDataPackage dataPackage) {
        ...
        logic.notify(ACTIONS.INCOMING_MESSAGE, new Object[]{
                sender,
    ...
    protected boolean onIncomingCall(AbstractDataPackage dataPackage) {
        ...
        if (user.isCalling() != ClientUser.NO_ONE) {
            ...
            try {
                ...
                logic.notify(ACTIONS.CALLED_BUT_BUSY, new Object[]{sender});
            } catch (IOException ignored) {
                //think about return false to indicate that you are failed in network
            }
            return true;
        }
        ...
        logic.notify(ACTIONS.INCOMING_CALL, new Object[]{
                sender, dudesInConv
        });
        return true;
    }
    ...
    protected boolean onSendSound(AbstractDataPackage dataPackage) {
        ...
        logic.notify(ACTIONS.INCOMING_SOUND, new Object[]{
    ...
    protected boolean onExitConversation(AbstractDataPackage dataPackage) {
        logic.notify(ACTIONS.EXITED_CONVERSATION, null);
    ...
    protected boolean onCallAccept(AbstractDataPackage dataPackage) {
        ...
        Algorithms.callAcceptRoutine(dude, dataPackage.getDataAsString(), logic, model);
        return true;
    }
    ...
    protected boolean onCallDeny(AbstractDataPackage dataPackage) {
        user.drop();
        BaseUser baseUser = model.getUserMap().get(dataPackage.getHeader().getFrom());
        logic.notify(ACTIONS.CALL_DENIED, new Object[]{baseUser});
        return true;
    }
    ...
    protected boolean onCallCanceled(AbstractDataPackage dataPackage) {
        user.drop();
        BaseUser baseUser = model.getUserMap().get(dataPackage.getHeader().getFrom());
        logic.notify(ACTIONS.CALL_CANCELLED, new Object[]{baseUser});
        return true;
    }
    ...
    protected boolean onBothInConversation(AbstractDataPackage dataPackage) {
        if (user.isCalling() == dataPackage.getHeader().getFrom())
            user.drop();
        logic.notify(ACTIONS.BOTH_IN_CONVERSATION, new Object[]{
